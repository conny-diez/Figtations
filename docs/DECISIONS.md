...
## D-003 · M7 spike — Route A built, then verified in the client; Route B dropped
...
**Resolved — verified in the desktop client (2026-08-13).** A human ran the
spike against the 1.0.0-beta.1 build. Route A works: a connector written via
`setVectorNetworkAsync` is editable in the native vector edit mode, the edits
read back, waypoints persist across `Done` and subsequent syncs, and bend-tool
tangents survive the round trip. Questions 1 and 2 — the two that could have
forced Route B — are answered in the affirmative, so **Route B is not built and
is now dropped rather than deferred.**

The verification was a functional pass, not an exhaustive one: the FR-5b
robustness cases in `docs/QA.md` (branched network, 13th waypoint, hard-kill with
an unlocked connector, PNG export artefacts, undo across a vector edit session)
were not walked through and stay open. What is settled is the architectural
question this entry existed to answer.
...
## D-015 · The manifest carries the real, Figma-assigned plugin id — ~~superseded~~ ~~revised~~ resolved

**Problem.** §4.3 specifies `"id": "TBD_AFTER_FIRST_PUBLISH"`, on the assumption
that Figma only hands out the real id when the plugin is published.
...
**Second decision (also wrong, in a quieter way).** Ship a syntactically valid
numeric id, `1417382967104512873`, and replace it on first publish. That number
was **invented by the implementing agent** to match the shape of a Figma id — it
was never issued by Figma. It unblocked `setRelaunchData()`, so the symptom
disappeared and the guess survived unexamined into the release preparation. A
made-up id in the field reserved for a real one is a placeholder wearing a
disguise: nothing fails, and nothing tells you it is wrong.

**Final decision.** The manifest carries `1669772164275856800`, generated by Figma
via _Plugins → Development → New plugin_. It does **not** get replaced later. The
plugin manifest documentation is explicit that this is already the permanent id:

> The plugin ID to publish updates to. This ID will be assigned to you by Figma
> and is typically obtained using the "Create new Plugin" feature, which will
> generate a manifest file with a new `id`. You can also get a new plugin ID at
> the time of publishing your plugin.

The second sentence is an alternative route for someone who never created a
plugin entry, not a replacement of an existing id — the publish flow only offers
_Generate ID_ when the manifest id is missing or invalid. The premise behind
§4.3's `TBD_AFTER_FIRST_PUBLISH` is therefore simply wrong, and every
"replace this after publishing" note has been removed from the changelog and the
release notes.

**Rationale.** The first decision inverted the risk: the field is required by a
runtime API, not just by the publish flow. The second decision fixed that but
substituted a fabricated value for a real one, which is worse than a placeholder
that announces itself — `TBD_AFTER_FIRST_PUBLISH` would at least have failed
loudly or been caught by eye. `setRelaunchData()` stays wrapped in try/catch
(D-020) regardless, so the relaunch button degrades instead of taking the card
with it.

**Source.** https://developers.figma.com/docs/plugins/manifest/ — retrieved
2026-08-13.
...
## D-032 · "(Beta)" lives in the manifest name only — temporary

**Problem.** The plugin list in Figma shows `manifest.name` and nothing else. A
tester who has both this and a later stable build installed cannot tell them
apart, and someone who installed it once has no reminder that they are running a
beta.

**Decision.** `manifest.name` is `Figtations (Beta)`. The suffix is **not**
repeated anywhere else:

- **Menu commands stay as they are** (`Open Figtations`, `Annotate selection`, …).
  Figma nests them under the plugin name, so the menu already reads
  _Figtations (Beta) → Open Figtations_. Repeating it would stutter.
- **The panel header stays `Figtations`.** The version badge sits directly beside
  it (`strings.version`) and reads `V1.0.0-BETA.1`, which is both more precise
  and already visible. "Figtations (Beta) V1.0.0-BETA.1" says the same thing
  three times.
- **README and prose keep the product name.** The product is Figtations; the
  beta is a state of the release, not a rename.

**Removal.** The suffix comes out for the first stable release. Concretely: when
a `1.0.0` (no prerelease segment) is cut, `manifest.name` goes back to
`Figtations` in the same commit that drops the `-beta.N` suffix from
`package.json`. Nothing else has to change, which is the point of confining it to
one field. Publishing to the Community under the beta name would carry the suffix
into the listing, so the rename has to precede the first publish.

**Rationale.** One field to set and one field to unset. The alternative — the
suffix in the manifest, the menu, the header and the docs — is four places to
remember and four places to get inconsistent, for information the version badge
already carries inside the plugin.
